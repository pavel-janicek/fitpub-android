...
| **0.4.7 (current)** | Patch — author card on activity detail: avatar + display name + @username in a tappable card at the top of the detail screen, with an always-visible Follow/Unfollow button (supporting request-to-follow for private accounts and no-cached-status first-time follow); fixes the 0.4.5 follow affordance that never rendered | ✅ done |
